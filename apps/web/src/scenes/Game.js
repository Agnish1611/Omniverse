...
export default class Game extends Phaser.Scene {
    ...
    setCurrentUser;
    setSocket;
    constructor(updateMessages, setCurrentUser, setSocket) {
        super('game');
        this.isJoined = false;
    ...
    moveOtherPlayer(id, x, y, direction) {
    ...
    idleOtherPlayer(id, x, y, direction) {
        const player = this.players[id];
        if (player) {
            const { sprite, label } = player;
            sprite.setPosition(x, y);
    ...
    receiveMessage(id, text) {
        if (this.messages.length === 1)
            this.messages[0] = { user: id, text: text };
        else
            this.messages.push({ user: id, text: text });
    ...
    update(t, dt) {
        ...
        if (this.cursors.left?.isDown) {
            ...
            if (currentAnim) {
                ...
                if (this.isJoined)
                    this.sendIdle(this.adam.x, this.adam.y, currentAnim[2]);
            }
